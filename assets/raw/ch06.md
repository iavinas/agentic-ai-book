...
```
<figcaption>Figure 6.1: ReAct agent architecture — LLM backend, tool system, and state feed into the ReAct loop.</figcaption>
</figure>
...
# Self-test: stub agent must fail every task
...
# Self-test: local backend instantiates with dummy key
local = LocalBackend()
assert local.model == "kimi-k2.6"
```
...
class _SafeEvaluator(ast.NodeVisitor):
    """AST-based safe math evaluator. Only allows basic arithmetic and whitelisted functions."""
    _ALLOWED = {
    ...
    def visit_Call(self, node):
        if not isinstance(node.func, ast.Name): raise ValueError("Only simple function calls allowed")
        if node.func.id not in self._ALLOWED: raise ValueError(f"Function '{node.func.id}' not allowed")
        args = [self.visit(arg) for arg in node.args]
        return self._ALLOWED[node.func.id](*args)
    def visit_Name(self, node):
        if node.id in self._ALLOWED: return self._ALLOWED[node.id]
...
# Self-test: the safe evaluator rejects malicious input and computes correct answers
assert calculator("15 * 23 + 47") == "392"
assert calculator("2 + 2 * sqrt(16)") == "10.0"
assert "Error" in calculator("__import__('os')")
```

The calculator parses the expression into an abstract syntax tree and evaluates only whitelisted nodes — numeric constants, arithmetic operators, and a small set of math functions. It never calls Python's `eval`, so malicious input like `__import__('os').system('rm -rf /')` is rejected at the AST level.
...
# Self-test: reading a missing file returns an error message
assert "not found" in read_file("/tmp/nonexistent_file_12345.txt").lower()
```
...
# Self-test: write a temp file and verify it exists
test_write_path = "/tmp/ch06_write_test.txt"
write_file(test_write_path, "hello")
assert os.path.exists(test_write_path)
```
...
# Self-test: python_execute runs a simple calculation
assert python_execute("print(2 ** 10)") == "1024"
```
...
# Self-test: schema is generated correctly
assert hasattr(web_search, "_tool_schema")
assert web_search._tool_schema["name"] == "web_search"
```

The `web_search` tool delegates to SerpAPI, a managed search-results API. You need a `SERPAPI_KEY` environment variable. If you prefer a different provider (Bing API, Tavily, DuckDuckGo), the implementation is identical: issue an HTTP request, extract snippets, and return them as a single string observation.
...
# Self-test: schemas are generated for both memory tools
assert memory_store._tool_schema["parameters"]["properties"]["key"]["type"] == "string"
assert memory_retrieve._tool_schema["name"] == "memory_retrieve"
```
...
```mermaid
flowchart TB
    SP["<b>System prompt</b><br/>Role, tools, output format"]
    SB["<b>Scratchpad</b><br/>Thought / Action / Observation history"]
    UT["<b>User task</b><br/>Goal sentence"]
    SP --> SB --> UT
```
<figcaption>Figure 6.2: ReAct prompt structure — system prompt, scratchpad history, and user task.</figcaption>
</figure>
...
- web_search(query): searches the web via SerpAPI.
...
class ToolCallParser:
    ...
    def parse(response: str) -> tuple:
        ...
        if action_text.lower().startswith("finalanswer"):
            if "(" in action_text:
                answer = action_text.split("(", 1)[1]
                # Strip exactly one trailing ) so answers containing parens are preserved
                if answer.endswith(")"):
                    answer = answer[:-1]
            else:
                answer = action_text
        ...
        if func_match:
            ...
            try:
                wrapped = "{" + args_text + "}" if not args_text.strip().startswith("{") else args_text
                args = json.loads(wrapped)
                return thought, {"tool": tool_name, "args": args}
            except json.JSONDecodeError:
                pass
            # Fallback: parse key=value pairs (handles single quotes and bare values)
            args = {}
            ...
            if args:
                return thought, {"tool": tool_name, "args": args}
            # Ultimate fallback: single string argument
            return thought, {"tool": tool_name, "args": {"expression": args_text.strip()}}

        return thought, None
```

The parser is intentionally lenient. It tries strict JSON first, falls back to a regex that extracts `key=value` pairs with either single or double quotes, and finally treats the entire payload as a single string argument. This three-tier fallback handles the output variance of local models without crashing the loop.

```python
# Self-test: parser handles JSON, single quotes, nested parens, and plain values
assert ToolCallParser.parse('Thought: x\nAction: calculator(expression="2+2")')[1] \
       == {"tool": "calculator", "args": {"expression": "2+2"}}
assert ToolCallParser.parse("Thought: x\nAction: calculator(expression='2+2')")[1] \
       == {"tool": "calculator", "args": {"expression": "2+2"}}
assert ToolCallParser.parse("Thought: x\nAction: FinalAnswer(The value is (42))")[1] \
       == {"final_answer": "The value is (42)"}
```
...
# Self-test: factory produces a fresh agent with 7 tool schemas
agent = make_agent()
...
```mermaid
flowchart TD
    B["<b>Baseline — stub agent</b><br/>0/6 passed | 0%<br/>All tasks FAIL at 1 step"]
    L["<b>After LLM backend (no tools)</b><br/>2/6 | 33%<br/>Tasks 1 and 4 PASS (in-context math)"]
    T["<b>After tool system (single-step)</b><br/>3/6 | 50%<br/>Tasks 1, 2, 4 PASS; multi-step tasks still FAIL"]
    R["<b>After ReAct loop</b><br/>6/6 | 100%<br/>Avg steps 2.2 — sequencing completes all tasks"]
    B --> L --> T --> R
```
<figcaption>Figure 6.3: Eval-Driven Development progression — each component is justified by a measurable score improvement.</figcaption>
</figure>
...
=== After Tool System (single-step tool calling) ===
Task 1: PASS (1 steps)
Task 2: PASS (1 steps)      ← write_file only; evaluator checks file directly
...
sequenceDiagram
    ...
    participant FS as write_file

    Note over Agent: Step 1 — compute area
    Agent->>Calc: calculator("3.14159 * 7 ** 2")
    Calc-->>Agent: Observation: 153.93791

    Note over Agent: Step 2 — persist result
    Agent->>FS: write_file("/tmp/circle_area.txt", "153.93791")
    FS-->>Agent: Observation: Successfully wrote

    Note over Agent: Step 3 — FinalAnswer with summary
```
<figcaption>Figure 6.4: Task 5 execution trace — the agent decomposes a multi-step task into calculate, write, and confirm.</figcaption>
</figure>
...
# Self-test: planner class inherits plan and run methods
assert hasattr(PlannerReActAgent, "plan")
assert hasattr(PlannerReActAgent, "run")
```
...
- [vLLM](https://github.com/vllm-project/vllm) — High-throughput inference engine for local and hosted LLMs.
- [llama.cpp](https://github.com/ggerganov/llama.cpp) — Portable C++ inference for Llama-based models across hardware.
