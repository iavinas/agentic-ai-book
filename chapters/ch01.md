...
A **predictor** generates a single forward pass:
...
class SimplePredictor(nn.Module):
    """A minimal transformer that emits text completions."""
...
# Usage: one forward pass, one answer.
model = SimplePredictor(vocab_size=32000)
tokens = torch.randint(0, 32000, (1, 128))  # (1, 128) dummy prompt
logits = model(tokens)  # (1, 128, 32000)
next_token = logits[:, -1, :].argmax(dim=-1)  # greedy decode
```

The predictor emits tokens. If the training data happened to include the fact that $\sqrt{123456789} \approx 11111.11106$, it might answer correctly. If not, it hallucinates. It has no way to know.
...
class MinimalAgent:
    """An agent that can call a calculator tool and verify results."""
    def __init__(self, llm_backend, tools):
        self.llm = llm_backend
        self.tools = tools
        self.trace = []  # (thought, action, observation) tuples

    def run(self, task: str, max_steps: int = 10):
        state = {"task": task, "done": False}
        for step in range(max_steps):
            # LLM decides: think, or call a tool, or answer
...
# Usage: the agent loops until it verifies the answer.
agent = MinimalAgent(llm_backend=None, tools={"calculator": calculator})
# The LLM would generate a thought like:
#   "I need sqrt(123456789). Let me call the calculator."
# Then observe: "11111.111060555522"
# Then generate a new thought:
#   "Now I must check if this is prime. sqrt(123456789) = 11111.111..."
#   "Since it is not an integer, it cannot be a perfect square."
#   "To check primality, I can test divisibility by small primes..."
```
