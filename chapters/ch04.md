...
Older libraries predate these engines but remain influential. **Outlines** (2023) pioneered FSM precomputation from regexes and JSON Schema and is still widely used in Python-only stacks. **guidance** (Microsoft) offers a templating language for interleaving prompts, control flow, and constrained blocks — better suited to multi-step reasoning than pure schema enforcement. **jsonformer** takes a simpler approach: it generates the structural tokens (braces, keys, colons) under constraint and leaves value tokens to free sampling, which keeps implementation trivial but handles only a subset of JSON Schema. All three have been largely superseded by XGrammar and llguidance for production inference, but they remain useful references for understanding the design space.
...
**Anthropic** enforces JSON Schema adherence natively in its tool use API without requiring `additionalProperties: false` or making all fields explicitly required. Empirical comparisons consistently report lower schema violation rates than OpenAI's non-strict default, though Anthropic does not expose a toggleable `strict` boolean.
...
Meta-tools are first-class citizens. They consume context tokens like any other tool, but they reduce the need to load the entire registry. An agent with 1,000 tools might load only the 10 retrieved tools plus the 2 meta-tools — 12 tool definitions total. OpenAI ships a provider-native equivalent called `tool_search`, available on `gpt-5.5` and later: the model calls a built-in search tool over a large tool catalog and loads only the returned definitions, deferring rarely used tools until they are needed.
