...
```mermaid
flowchart TB
    subgraph AgentState["Agent State (Step k)"]
        direction TB
        G["Goal: \"Calculate Q3 revenue growth\""]
        TL["Tools: [search, calculator, file_read]"]
        TR["Trace"]
        TR1["Step 1: thought → action → obs"]
        TR2["Step 2: thought → action → obs"]
        DOTS["..."]
        TRK["Step k-1: thought → action → obs"]
        G --> TL
        TL --> TR
        TR --> TR1
        TR1 --> TR2
    ...
    subgraph Env["Environment (External)"]
        EV["Web, Files, APIs, Databases, Users"]
    end
...
*Figure 2.4 — Evolution from classic ReAct to production ReAct. The 2026 loop adds parallel actions, step-level verifiers, and repair thoughts, but the core interleaved reasoning-acting structure remains.*
...
```mermaid
flowchart TD
    L3["<b>Trajectory Level (Reflexion)</b><br/>After full run fails, write verbal<br/>self-reflection to memory and retry."]
    L2["<b>Sub-Task Level (CRITIC)</b><br/>Ground claims against external evidence<br/>before committing side effects."]
    L1["<b>Step Level (ReAct + Verifier)</b><br/>Cheap, fast verification gates every<br/>action before execution."]
    L3 --> L2 --> L1
```

*Figure 2.5 — Three-layer self-correction architecture in 2026 production agents. Errors are caught at the cheapest level possible, with higher layers handling more complex failures.*
