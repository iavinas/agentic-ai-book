...
layout: chapter
chapter: 9
title: "Tree and Graph of Thoughts"
part: "Part II: Single-Agent Systems"
prev: /chapters/ch08.html
prev_title: "Chain-of-Thought and Advanced Reasoning"
next: /chapters/ch10.html
next_title: "Code Generation Agents"
---
...
flowchart LR
    subgraph CoT["Chain-of-Thought (Linear)"]
        C1["Step 1: 17 × 30 = 510"] --> C2["Step 2: 17 × 8 = 136"]
        C2 --> C3["Step 3: 510 + 136 = 646"]
        C3 --> C4["Answer: 646"]
    end
    subgraph ToT["Tree of Thoughts (Branching)"]
        T1["Root: numbers 4, 9, 10, 13"] --> T2a["9 + 13 = 22"]
        T1 --> T2b["13 - 9 = 4"]
        T1 --> T2c["10 × 4 = 40"]
        T2a --> T3a["22 + 10 = 32"]
        T2b --> T3b["4 × 4 = 16"]
    ...
    style CoT fill:#185FA5,color:#fff
    style ToT fill:#0F6E56,color:#fff
```

<figcaption>Figure 9.1 — CoT commits to a single linear scratchpad. ToT generates a branching tree of candidate intermediate steps and searches over them.</figcaption>
</figure>
...
```mermaid
flowchart TD
    subgraph BFS["Breadth-First (Beam) Search"]
        B1["Level 0"] --> B2a["Level 1a"]
        B1 --> B2b["Level 1b"]
        B1 --> B2c["Level 1c"]
        B2a --> B3a["Level 2a"]
        B2b --> B3b["Level 2b"]
    ...
    subgraph DFS["Depth-First Search"]
        ...
        D2 --> D3["Grandchild A1"]
        D3 --> D4["Leaf (dead end)"]
        D4 -.-> D5["Backtrack to Child B"]
        D5 --> D6["Child B"]
    end
    style BFS fill:#534AB7,color:#fff
    style DFS fill:#854F0B,color:#fff
```

<figcaption>Figure 9.2 — BFS explores all candidates at a given depth before deepening. DFS commits to one line of reasoning and backtracks only when it fails.</figcaption>
</figure>
...
```mermaid
flowchart TD
    Root["Problem"] --> T1["Tree 1 (algebraic)"]
    Root --> T2["Tree 2 (numeric)"]
    Root --> T3["Tree 3 (factorization)"]
    T1 --> A1["Active"]
    T2 --> A2["Pruned early"]
    T3 --> A3["Active"]
    A1 --> C1["Candidate answer A"]
    A3 --> C2["Candidate answer A"]
    C1 --> Consensus["Consensus = Answer A"]
    C2 --> Consensus
    style A2 fill:#993C1D,color:#fff
```

<figcaption>Figure 9.3 — Forest-of-Thought runs parallel reasoning trees, deactivating low-confidence ones early and resolving the remainder via consensus.</figcaption>
</figure>
...
```mermaid
flowchart TD
    Root["Query"] --> S1["Subproblem A"]
    Root --> S2["Subproblem B"]
    S1 --> N1["Nested subgraph"]
    S1 --> N2["Nested subgraph"]
    S2 --> N2
    N2 --> N3["Refinement"]
    N3 --> Ans["Answer"]
    style Root fill:#534AB7,color:#fff
    style Ans fill:#0F6E56,color:#fff
```

<figcaption>Figure 9.4 — AGoT dynamically decomposes a query into nested subgraphs. Subproblems that need deeper analysis spawn their own local reasoning DAGs.</figcaption>
</figure>
...
```mermaid
flowchart LR
    U["Understand<br/>(decompose)"] --> R["Reflect<br/>(validate)"]
    R --> C["Code<br/>(verify)"]
    C --> S["Summary<br/>(synthesize)"]
    U -.->|forbidden| U
    R -.->|forbidden| R
    style U fill:#185FA5,color:#fff
    style R fill:#854F0B,color:#fff
    style C fill:#0F6E56,color:#fff
    style S fill:#534AB7,color:#fff
```

<figcaption>Figure 9.5 — CMCTS constrains the MCTS action space to four disjoint reasoning modes, with partial-order rules preventing redundant sequences.</figcaption>
</figure>
...
flowchart TD
    subgraph Aggregation["Aggregation"]
        A1["Thought A<br/>(fast but approximate)"] --> M["Merged Thought"]
        B1["Thought B<br/>(slow but exact)"] --> M
    end
    subgraph Revision["Revision"]
        O["Original Thought<br/>(contains error)"] --> R["Revised Thought"]
        R --> C["Child 1"]
        R --> D["Child 2"]
        O -.->|pruned in tree| X["Lost subtree"]
    end
    style M fill:#534AB7,color:#fff
    style R fill:#0F6E56,color:#fff
    style X fill:#993C1D,color:#fff
```

<figcaption>Figure 9.6 — GoT aggregation merges insights from parallel branches. GoT revision edits a node in place, preserving its descendants rather than pruning the entire subtree.</figcaption>
</figure>
...
## Agentic Code Project: LangGraph Tree-of-Thought Solver

Section 4 built `beam_search_tot` in pure Python. The **production project** in [`chapters/projects/ch09-tot-solver/`](../projects/ch09-tot-solver/) implements the same ideas as a **LangGraph** state machine with optional **Graph-of-Thought** merge, OpenAI-compatible APIs, and Ollama support.

### What you will run

| Mode | Engine | API key |
|------|--------|---------|
| `--mode rule` | Complete BFS over arithmetic states | None |
| `--mode llm` | LangGraph beam loop (`expand → evaluate → prune → …`) | `OPENAI_API_KEY` or `--use-ollama` |

```bash
conda activate agentic-ai-book   # shared book conda env
cd chapters/projects/ch09-tot-solver
pip install -r requirements.txt

# Offline (CI-friendly)
python -m tot_solver 4 9 10 13 --mode rule

# OpenAI-compatible API
export OPENAI_API_KEY=sk-...
python -m tot_solver 4 9 10 13 --mode llm --beam-width 3 --trace ./runs/trace.jsonl

# Local Ollama (default model: kimi-k2.6)
python -m tot_solver 4 9 10 13 --mode llm --use-ollama
```

### LangGraph topology

```text
expand → evaluate → prune → [got_merge] → route → expand | finish
```

- **expand** — LLM proposes one arithmetic step per call (structured JSON).
- **evaluate** — LLM-as-judge scores promise (1–10 or sure/maybe/impossible).
- **prune** — beam-select top-*k* branches; detect goal states.
- **got_merge** (optional, `--got-merge`) — merge the top two frontier branches before continuing.
- **trace** — JSONL events for expand/evaluate/prune (`--trace path.jsonl`).

Rule mode uses **BFS** so the eval suite is deterministic; LLM mode exercises the LangGraph loop and is where beam width and evaluator quality matter in production.

See the project [README](../projects/ch09-tot-solver/README.md) for configuration, pytest, and the regression suite (`python -m tot_solver.eval_suite`).

The trade-off from Section 4 still holds: rule/BFS search is cheap and complete for Game of 24; LLM ToT is slower but generalizes to any state you can describe in natural language. Production systems cache evaluator scores and use lightweight PRMs when the domain allows.
...
- [Large Language Model Guided Tree-of-Thought](https://arxiv.org/abs/2305.08291) — Long, 2023. A parallel ToT formulation that unifies tree-structured prompting under a single scaffold.
...
- [Atom of Thoughts for Markov LLM Test-Time Scaling](https://arxiv.org/abs/2502.12018) — Teng et al., NeurIPS 2025. Markovian reasoning decomposition for efficient test-time scaling.
