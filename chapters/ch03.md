...
assert self_consistency_vote(traces, extract) == "391"
print("Self-consistency test passed")  # 391
```

### 1.3 The 2025–2026 Frontier: Hierarchical and Fractured Reasoning

The flat, linear CoT of 2022 has evolved. In 2026, **Hierarchical Chain-of-Thought (Hi-CoT)** replaces the single chain with an alternating instruction–execution structure. The model emits `<|instruction|>` tokens to plan the next subgoal, then `<|execution|>` tokens to carry it out. This creates compression bottlenecks where the model must distill its current state into a concise subgoal before proceeding. Across diverse model configurations, Hi-CoT improves average accuracy by 6.2% while *reducing* reasoning trace length by 13.9%, proving that structure — not just more tokens — is the primary bottleneck.
...
# Test: in a vocab of 5 tokens, only token 0 survives
logits = torch.tensor([[0.2, 0.5, -0.1, 0.3, 0.0]])
masked = constrain_tool_logits(logits, valid_tokens=[0], vocab_size=5)
expected = torch.tensor([[0.2, float('-inf'), float('-inf'), float('-inf'), float('-inf')]])
assert torch.allclose(masked, expected, equal_nan=True)
print("Constrained decoding test passed")
```
...
An **open tool set** allows the agent to discover new tools during execution. The agent might browse a tool registry, read API documentation, or even write new tools as code. This is the paradigm of Toolformer and Voyager: the agent learns *how* to use tools by observing examples. **Gorilla** (Patil et al., 2023) showed that LLMs can be fine-tuned to accurately invoke thousands of APIs by learning from documentation and call examples. **ToolLLM** (Qin et al., 2023) scaled this to 16,000+ real-world APIs, providing the training corpus and benchmark that enabled open-weight models to match proprietary tool-calling performance. Open tool sets are powerful but dangerous — the action space is unbounded, and a malicious or poorly described tool can hijack the agent's behavior.
...
Two strategies dominate. **Plan-then-execute** generates a full plan before any action. The LLM is prompted to produce a sequence of steps, and then each step is executed in order. The **Plan-and-Solve** strategy (Wang et al., 2023) formalizes this by explicitly separating plan generation from plan execution in the prompt, improving zero-shot reasoning on complex problems. This is efficient when the environment is stable and the plan is unlikely to need revision. It fails when the environment is dynamic: a web page changes, a file is missing, or an API is down.
...
**Least-to-Most prompting** (Zhou et al., 2023) offers a complementary decomposition strategy: the model solves the simplest subproblem first, then progressively builds up to harder ones, reusing intermediate results. This mirrors human problem-solving and is especially effective when subproblems have clear dependencies.
...
2026 research has shifted the emphasis from ingestion to retrieval. A systematic ablation on the **LongMemEval** benchmark (from the **MemMachine** study) shows that retrieval-depth tuning (+4.2% accuracy) and context formatting (+2.0%) outweigh ingestion-side changes like sentence chunking (+0.8%). This means systems over-investing in heavy LLM-based fact extraction during ingestion may be optimizing the wrong stage.
...
results = rag.retrieve("How does an agent reason step by step?", k=2)
assert len(results) == 2
print("MinimalRAG retrieval test passed")
```
...
- [Hierarchical Chain-of-Thought Prompting](https://arxiv.org/abs/2604.00130) — Huang et al., 2026. Hi-CoT: alternating instruction–execution structure for more efficient reasoning.
- [Fractured Chain-of-Thought Reasoning](https://arxiv.org/abs/2505.12992) — Liao et al., 2025. Truncated and depth-interpolated sampling for better accuracy per token.
...
- [ToolLLM: Facilitating Large Language Models to Master 16,000+ Real-world APIs](https://arxiv.org/abs/2307.16789) — Qin et al., 2023. Scaling open tool discovery to massive API ecosystems.
- [From Coarse to Fine: Self-Adaptive Hierarchical Planning for LLM Agents](https://arxiv.org/abs/2604.23194) — Tan et al., 2026. AdaPlan-H: adaptive planning granularity via imitation learning.
- [HiPER: Hierarchical Reinforcement Learning with Explicit Credit Assignment for LLM Agents](https://arxiv.org/abs/2602.16165) — Peng et al., 2026. Macro-micro credit assignment for hierarchical agent RL.
- [Gorilla: Large Language Model Connected with Massive APIs](https://arxiv.org/abs/2305.15334) — Patil et al., 2023. Fine-tuning LLMs to invoke thousands of APIs from documentation.
- [Plan-and-Solve Prompting: Improving Zero-Shot Chain-of-Thought Reasoning by Large Language Models](https://arxiv.org/abs/2305.04091) — Wang et al., 2023. Explicitly separating plan generation from execution in the prompt.
- [Least-to-Most Prompting Enables Complex Reasoning in Large Language Models](https://arxiv.org/abs/2205.10625) — Zhou et al., 2023. Solving simple subproblems first, then building up to harder ones.
- [Reason-Plan-ReAct: A Reasoner-Planner Supervising a ReAct Executor for Complex Enterprise Tasks](https://arxiv.org/abs/2512.03560) — Molinari & Ciravegna, 2025. Decoupling high-level planning from low-level execution.
- [MemMachine: A Ground-Truth-Preserving Memory System for Personalized AI Agents](https://arxiv.org/abs/2604.04853) — Wang et al., 2026. Retrieval-stage ablations on LongMemEval showing depth tuning dominates chunking.
- [Active Context Compression: Autonomous Memory Management in LLM Agents](https://arxiv.org/abs/2601.07190) — Verma, 2026. The Focus system: agent-controlled compression with 22.7% token reduction on SWE-bench Lite.
- [Dynamic Long Context Reasoning over Compressed Memory via End-to-End Reinforcement Learning](https://arxiv.org/abs/2602.08382) — Chen et al., 2026. Chunk-wise compressor trained with RL, extrapolating to 1.75M tokens.
- [MemPrompt: Memory-assisted Prompt Editing with User Feedback](https://arxiv.org/abs/2201.06009) — Wu et al., 2022. Editing prompts based on stored user feedback.
- [MemoryBank: Enhancing Large Language Models with Long-Term Memory](https://arxiv.org/abs/2305.10250) — Zhong et al., 2023. Structured long-term memory for conversational agents.
- [SimpleMem: Efficient Lifelong Memory for LLM Agents](https://arxiv.org/abs/2601.02553) — Liu et al., 2026. Semantic lossless compression and intent-aware retrieval.
- [GAM-RAG: Gain-Adaptive Memory for Evolving Retrieval](https://arxiv.org/abs/2603.01783) — Wang et al., 2026. Training-free adaptive retrieval with experience accumulation.
