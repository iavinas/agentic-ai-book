...
*Figure 6.1: ReAct agent architecture — LLM backend, tool system, and state feed into the ReAct loop.*
...
```python
class StubAgent:
    """An agent that always returns 'I don't know.' Every task should fail."""
...
*Figure 6.2: ReAct prompt structure — system prompt, scratchpad history, and user task.*
...
*Figure 6.3: Eval-Driven Development progression — each component is justified by a measurable score improvement.*
...
*Figure 6.4: Task 5 execution trace — the agent decomposes a multi-step task into calculate, write, and confirm.*
