...
<figcaption>Figure 13.1 — Static RAG (left) retrieves once and generates in a single pass. Agentic RAG (right) wraps retrieval inside a decide-rewrite loop, iterating until the evidence buffer suffices, then synthesizes with citations.</figcaption>
...
```mermaid
flowchart LR
    Q[Question:<br/>population of the capital<br/>of the 2022 World Cup winner] --> H1
    subgraph H1["Hop 1"]
        R1[Retrieve:<br/>2022 World Cup winner] --> D1[Doc: Argentina won]
    end
    D1 --> G[Gist memory:<br/>winner = Argentina]
    G --> RW1[Rewrite:<br/>capital of Argentina]
    RW1 --> H2
    subgraph H2["Hop 2"]
        R2[Retrieve:<br/>capital of Argentina] --> D2[Doc: Buenos Aires]
    end
    D2 --> G2[Gist:<br/>capital = Buenos Aires]
    G2 --> RW2[Rewrite:<br/>population of Buenos Aires]
    RW2 --> H3
    subgraph H3["Hop 3"]
        R3[Retrieve:<br/>population of Buenos Aires] --> D3[Doc: ~15 million]
    end
    D3 --> Syn[Synthesize + cite]
```

<figcaption>Figure 13.3 — A three-hop evidence chain. Each hop's finding is condensed into gist memory, which seeds the next query rewrite. Without the gist, the agent loses the thread after hop 2.</figcaption>
...
The 2025 literature has refined HyDE in several directions. **HyPE** (Hypothetical Prompt Embeddings) inverts the workflow: at index time, the system generates hypothetical questions for every document chunk and stores their embeddings. At query time, no LLM call is needed; the user's question is matched directly against the pre-computed question vectors. This trades index size (3–5x more vectors) for zero query-time latency. **Self-Learning HyDE** (SL-HyDE) iteratively improves both the generator and the retriever using only unlabeled corpora, surpassing HyDE by an average of 4.9% NDCG@10 across ten datasets on the CMIRB medical retrieval benchmark. In practice, a robust agentic strategy is to ensemble: generate 3–5 hypothetical documents, average their embeddings, and retrieve against the centroid.
...
A retrieval agent that does not cite its sources is not trustworthy. In 2025–2026, the research community has converged on a sharp distinction between **citation correctness** (does the cited passage entail the claim?) and **citation faithfulness** (did the model actually derive the claim from that passage?). Production RAG often fails the second test. Wu et al. (2025) report that 50–90% of LLM responses are not fully supported by the sources they cite, and Wallat et al. (2024) find up to 57% of citations on adversarial sets are unfaithful — the model retrieved real documents but attributed a parametric-memory answer to them post-hoc.
...
<figcaption>Figure 13.2 — The RAGAgent architecture. The Decision Module reads the evidence buffer and routes back to the Query Rewriter (continue) or forward to the Synthesizer (answer). The Vector Store supplies candidates; the Synthesizer consumes the accumulated evidence.</figcaption>
